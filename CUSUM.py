...
plt.plot(range(1, len(cusum_df) + 1), cusum_df['C+'], color='red', label='C+ (накопление превышения)')
plt.plot(range(1, len(cusum_df) + 1), cusum_df['C-'], color='blue', label='C- (накопление снижения)')
plt.axhline(H, linestyle='--', color='black', label='Контрольный предел H')
plt.title('CUSUM-график: Среднее время ответа')
plt.plot(v_mask_x, v_mask_y, linestyle='--', color='black', label='V-маска')
plt.xlabel('День')
plt.ylabel('Накопленное отклонение (сек.)')
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()